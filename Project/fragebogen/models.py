...
class FragebogenAntwort(models.Model):
    ...
    jugendliche_person = models.ForeignKey(
        JugendlichePerson,
        on_delete=models.CASCADE,
        related_name="fragebogen_antworten"
    )

    bewertet_von = models.CharField(
        max_length=20,
        choices=[
    ...
    def __str__(self):
        return (
            f"{self.fragebogen.titel} - "
            f"{self.jugendliche_person.vorname} "
            f"{self.jugendliche_person.nachname}"
        )
